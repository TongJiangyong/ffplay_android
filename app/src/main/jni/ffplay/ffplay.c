...
static const struct TextureFormatEntry {
	...
	int texture_fmt;
} sdl_texture_format_map[] = { AV_PIX_FMT_NONE, 0 };

#if CONFIG_AVFILTER
static int opt_add_vfilter(void *optctx, const char *opt, const char *arg) {
	//GROW_ARRAY(vfilters_list, nb_vfilters);
      vfilters_list = (char const**)grow_array((void*)vfilters_list, sizeof(*vfilters_list), &nb_vfilters, nb_vfilters + 1);
		...
		if (pkt.data == flush_pkt.data) {
			...
			if (d->avctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {
				...
				if (ret < 0) {
					...
					ret = got_frame ?
							0 : (pkt.data ? AVERROR(EAGAIN) : AVERROR_EOF );
				}
			} else {
...
static int upload_texture(SDL_Texture **tex, AVFrame *frame,
	...
	if (realloc_texture(tex,
			sdl_pix_fmt == SDL_PIXELFORMAT_UNKNOWN ?
					SDL_PIXELFORMAT_ARGB8888 : sdl_pix_fmt, frame->width,
	...
	case SDL_PIXELFORMAT_UNKNOWN:
		...
		*img_convert_ctx = sws_getCachedContext(*img_convert_ctx, frame->width,
				frame->height, (enum AVPixelFormat)frame->format, frame->width, frame->height,
				(enum AVPixelFormat)AV_PIX_FMT_BGRA, sws_flags, NULL, NULL, NULL);
...
static void video_image_display(VideoState *is) {
	...
	SDL_RenderCopyEx(renderer, is->vid_texture, NULL, &rect, 0, NULL,
			vp->flip_v ? (SDL_RendererFlip)SDL_FLIP_VERTICAL : (SDL_RendererFlip)0);
	if (sp) {
...
static void video_audio_display(VideoState *s) {
	...
	if (!s->paused) {
		...
		if (s->show_mode == SHOW_MODE_WAVES) {
			...
			for (i = 0; i < 1000; i += channels) {
				...
				int b =
						s->sample_array[(idx + 4 * channels) % SAMPLE_ARRAY_SIZE];
				int c =
						s->sample_array[(idx + 5 * channels) % SAMPLE_ARRAY_SIZE];
				int d =
						s->sample_array[(idx + 9 * channels) % SAMPLE_ARRAY_SIZE];
				int score = a - d;
				if (h < score && (b ^ c) < 0) {
					h = score;
					i_start = idx;
	...
	if (s->show_mode == SHOW_MODE_WAVES) {
		...
		if (!s->rdft || !s->rdft_data) {
			...
			if (!SDL_LockTexture(s->vis_texture, &rect, (void **) &pixels,
					&pitch)) {
				pitch >>= 2;
...
static void check_external_clock_speed(VideoState *is) {
	if (is->video_stream
			...
			is->audio_stream >= 0 && is->audioq.nb_packets <= EXTERNAL_CLOCK_MIN_FRAMES) {set_clock_speed
		(&is->extclk,
...
static void update_volume(VideoState *is, int sign, double step) {
	double volume_level =
			is->audio_volume ?
					(20 * log(is->audio_volume / (double) SDL_MIX_MAXVOLUME)
							/ log(10)) :
					-1000.0;
	...
	is->audio_volume = av_clip(
			is->audio_volume == new_volume ?
					(is->audio_volume + sign) : new_volume, 0,
			SDL_MIX_MAXVOLUME);
}
...
static void video_refresh(void *opaque, double *remaining_time) {
	...
	if (is->video_st) {
		retry: if (frame_queue_nb_remaining(&is->pictq) == 0) {
...
static int configure_filtergraph(AVFilterGraph *graph, const char *filtergraph,
	...
	fail: avfilter_inout_free(&outputs);
...
static int configure_video_filters(AVFilterGraph *graph, VideoState *is,
	...
	ret = avfilter_graph_create_filter(&filt_out,
			avfilter_get_by_name("buffersink"), "ffplay_buffersink", NULL, NULL,
			graph);
	if (ret < 0)
		goto fail;

	if ((ret =
			av_opt_set_int_list(filt_out, "pix_fmts", pix_fmts, AV_PIX_FMT_NONE, AV_OPT_SEARCH_CHILDREN)) < 0)goto
		fail;
...
static int configure_audio_filters(VideoState *is, const char *afilters,
	...
	ret = avfilter_graph_create_filter(&filt_asrc,
			avfilter_get_by_name("abuffer"), "ffplay_abuffer", asrc_args, NULL,
			is->agraph);
	if (ret < 0)
		goto end;

	ret = avfilter_graph_create_filter(&filt_asink,
			avfilter_get_by_name("abuffersink"), "ffplay_abuffersink", NULL,
			NULL, is->agraph);
	if (ret < 0)
		goto end;

	if ((ret =
			av_opt_set_int_list(filt_asink, "sample_fmts", sample_fmts, AV_SAMPLE_FMT_NONE, AV_OPT_SEARCH_CHILDREN)) < 0)goto
		end;
	...
	if (force_output_format) {
		...
		if ((ret = av_opt_set_int(filt_asink, "all_channel_counts", 0,
			...
			goto end;
		if ((ret =
				av_opt_set_int_list(filt_asink, "channel_layouts", channel_layouts, -1, AV_OPT_SEARCH_CHILDREN)) < 0)goto
			end;
		if ((ret =
				av_opt_set_int_list(filt_asink, "channel_counts" , channels , -1, AV_OPT_SEARCH_CHILDREN)) < 0)goto
			end;
		if ((ret =
				av_opt_set_int_list(filt_asink, "sample_rates" , sample_rates , -1, AV_OPT_SEARCH_CHILDREN)) < 0)goto
			end;
	}
	...
	do {
		...
		if (got_frame) {
			...
			reconfigure = cmp_audio_fmts(is->audio_filter_src.fmt,
					is->audio_filter_src.channels, (enum AVSampleFormat)frame->format,
					frame->channels)
			...
			if (reconfigure) {
				...
				av_log(NULL, AV_LOG_DEBUG,
						...
						av_get_sample_fmt_name((enum AVSampleFormat)frame->format), buf2,
	...
	for (;;) {
		...
		if (last_w != frame->width || last_h != frame->height
			...
			av_log(NULL, AV_LOG_DEBUG,
					...
					(const char *) av_x_if_null(
							av_get_pix_fmt_name((enum AVPixelFormat)frame->format), "none"),
...
#endif
			duration = (
					frame_rate.num && frame_rate.den ? av_q2d((AVRational ) {
...
static int audio_decode_frame(VideoState *is) {
	...
	data_size = av_samples_get_buffer_size(NULL, af->frame->channels,
			af->frame->nb_samples, (enum AVSampleFormat )af->frame->format, 1);
	...
	if (af->frame->format != is->audio_src.fmt
		...
		is->swr_ctx = swr_alloc_set_opts(NULL, is->audio_tgt.channel_layout,
				...
				(enum AVSampleFormat )af->frame->format, af->frame->sample_rate, 0, NULL);
		...
		if (infinite_buffer < 1
				&& (is->audioq.size + is->videoq.size + is->subtitleq.size
						...
						|| (stream_has_enough_packets(is->audio_st,
								...
								&& stream_has_enough_packets(is->video_st,
...
static void stream_cycle_channel(VideoState *is, int codec_type) {
	...
	av_log(NULL, AV_LOG_INFO, "Switch %s stream from #%d to #%d\n",
			av_get_media_type_string((enum AVMediaType)codec_type), old_index, stream_index);
...
static void seek_chapter(VideoState *is, int incr) {
	...
	stream_seek(is,
			av_rescale_q(is->ic->chapters[i]->start,
					is->ic->chapters[i]->time_base, AV_TIME_BASE_Q), 0, 0);
}
...
static int opt_show_mode(void *optctx, const char *opt, const char *arg) {
	show_mode =
			!strcmp(arg, "video") ? SHOW_MODE_VIDEO :
